...
/// # Panic
///
/// Will panic if we failed to register a function with sqlite3
pub async fn register(connection: &mut sqlx::SqliteConnection) {
    // we're going to have to glue this together in this function to create a custom regex
    let mut handle = connection
        .lock_handle()
        .await
        .expect("Could not get a lock handle on the connection");
    let sqlite3 = handle.as_raw_handle();
    ...
    assert!(
        res == 0,
        "Could not register regex function: {:?}",
        ffi::Error::new(res)
    );
}
...
unsafe extern "C" fn sqlite3_regexp_func(
    ...
    if n_arg != 2 {
        dbg!("n_arg expected to be 2, is {n_arg}");
...
unsafe fn get_regex_from_arg(
    ...
    let regex = match Regex::new(value) {
        ...
        Err(e) => {
            eprintln!("Invalid regex {value:?}: {e:?}");
            ffi::sqlite3_result_error_code(ctx, ffi::SQLITE_CONSTRAINT_FUNCTION);
            return None;
        }
    };
...
unsafe fn get_text_from_arg<'a>(
    ...
    if ty == ffi::SQLITE_TEXT {
        ...
        match std::str::from_utf8(slice) {
            ...
            Err(e) => {
                eprintln!("Incoming text is not valid UTF8: {e:?}",);
                ffi::sqlite3_result_error_code(ctx, ffi::SQLITE_CONSTRAINT_FUNCTION);
                None
            }
        }
